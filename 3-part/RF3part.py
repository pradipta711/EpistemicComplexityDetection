...
class RF3partC:
    ...
    def __init__(self):
        print("Inside constructor RF3part")
    ...
    def preprocess_raw_text(self,raw_text):
        ...
        lemmatized_raw_word_list = [lemmatizer.lemmatize(w,self.get_wordnet_pos(w)) for w in important_words]
    ...
    def execute3part(self):
        ...
        #trainingDF = pd.read_csv("Soil_data_training.csv")
        #testingDF =pd.read_csv("Soil_testing.csv")
        ...
        randomForest = RandomForestClassifier(n_estimators=100,random_state=15325)
        randomForest.fit(X, y)

        y_predFE = randomForest.predict(X_testFE)
        ...
        randomForest1 = RandomForestClassifier(n_estimators=100,random_state=15325)
        randomForest1.fit(X1, y1)

        y_predEFE = randomForest1.predict(X_testEFE)
        ...
        randomForest2 = RandomForestClassifier(n_estimators=100,random_state=15325)
        randomForest2.fit(X2, y2)

        y_predEEE = randomForest2.predict(X_testEEE)
